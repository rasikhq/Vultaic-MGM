...
function aAdminMenu ()
	if ( aAdminForm == nil ) then
		...
		aTab1.ArenaOnly= guiCreateCheckBox ( 0.037, 0.90, 0.20, 0.04, "Arena Only", true, true, aTab1.Tab )
		aTab1.PlayerList		= guiCreateGridList ( 0.03, 0.10, 0.20, 0.80, true, aTab1.Tab )
...
function aClientGUIChanged ()
	if ( source == aTab1.PlayerListSearch ) then
		...
		local range = guiCheckBoxGetSelected(aTab1.ArenaOnly) and getElementParent(localPlayer) or root
		if ( text == "" ) then
			for id, player in ipairs ( getElementsByType ( "player", range ) ) do
	...
	if ( button == "left" ) then
		...
		if ( getElementParent ( source ) == aTab1.Tab ) then
			...
			elseif ( source == aTab1.HideColorCodes ) then
				updateColorCodes()
			elseif ( source == aTab1.ArenaOnly ) then
				updatePlayerList()
...
function updatePlayerList()
	guiGridListClear ( aTab1.PlayerList )
	for id, player in ipairs ( getElementsByType ( "player", guiCheckBoxGetSelected(aTab1.ArenaOnly) and getElementParent(localPlayer) or root ) ) do guiGridListSetItemPlayerName ( aTab1.PlayerList, guiGridListAddRow ( aTab1.PlayerList ), 1, getPlayerName ( player ), false, false ) end
end
