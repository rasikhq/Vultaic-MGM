...
function aAddUnmuteTimer( player, length )
	aRemoveUnmuteTimer( player )
	local serial = getPlayerSerial( player )
	aUnmuteTimerList[serial] = length > 0 and setTimer(
								function()
									aUnmuteTimerList[serial] = nil
...
function aRemoveUnmuteTimer( player )
	local serial = getPlayerSerial( player )
	if aUnmuteTimerList[serial] then
		if( isTimer( aUnmuteTimerList[serial] ) ) then
...
local function getPlayerNameClean( element )
	local _name = getPlayerName( element )
	local clean_name = string.gsub(_name, "#%x%x%x%x%x%x", "")
	return (clean_name or _name)
end
...
function aAction ( type, action, admin, player, data, more )
	if ( aLogMessages[type] ) then
		function aStripString ( string )
			...
			string = string.gsub ( string, "$admin", getPlayerNameClean ( admin ) )
			string = string.gsub ( string, "$by_admin_4all", isAnonAdmin4All( admin )    and "" or " by " .. getPlayerNameClean ( admin ) )
			string = string.gsub ( string, "$by_admin_4plr", isAnonAdmin4Victim( admin ) and "" or " by " .. getPlayerNameClean ( admin ) )
			string = string.gsub ( string, "$data2", more or "" )
			if ( player ) then string = string.gsub ( string, "$player", getPlayerNameClean ( player ) ) end
