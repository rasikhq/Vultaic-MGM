...
function setPlayerState(player, state)
	if isElement(player) then
		...
		triggerClientEvent(arena.element, "onClientArenaPlayerStateChange", resourceRoot, player, state)
	end
end
...
function(player, command, ...)
	...
	if not hasObjectPermissionTo(player, "function.kickPlayer", false) then
		if arena.nextMap then
			local mapName = arena.mapNames[arena.nextMap]
			if mapName then
				outputChatBox("Next map is "..arena.mapNames[arena.nextMap], source, 255, 255, 255)
			end
		end
		return
	end
	if arena.nextMap and not isObjectInACLGroup("user."..getAccountName(getPlayerAccount(player)), aclGetGroup("Admin")) then
	...
	if arena.mapNames[query] then
		...
		for i, v in pairs(arena.mapNames) do
			...
			if mapName:find(query, 1, true) then
