...
local defaultFont = {button = dxlib.getFont("Roboto-Medium", 11), gridlist = dxlib.getFont("RobotoCondensed-Regular", 12)}
...
function dxlib.renderContainer(container, renderTarget)
	...
	if container.items["gridlist"] then
		for i, item in pairs(container.items["gridlist"]) do
			...
			if y + item.height > 0 and y <= container.height then
				...
				if item.rows then
					local blockColor = item.blockColor or {255, 255, 255, 0}
	...
	local backgroundColor = container.backgroundColor or tocolor(15, 15, 15, 205)
	...
	if container.items["scrollbar"] then
		for i, item in pairs(container.items["scrollbar"]) do
			...
			dxDrawImage(x + (item.width - 16) * progress, y + (item.height - 16)/2, 16, 16, "img/dxlib/selector.png", 0, 0, 0, tocolor(255, 255, 255, 255))
		end
	end
	-- Buttons
	...
	if container.items["input"] then
		for i, item in pairs(container.items["input"]) do
			...
			if y + item.height > 0 and y <= container.height then
				...
				local backgroundColor = item.backgroundColor or {25, 25, 25, 205}
				local fontScale = item.fontScale or dxlib.fontScale
				local font = item.font or dxlib.getFont("Roboto-Medium", 13)
				...
				if backgroundColor[4] and backgroundColor[4] > 0 then
					dxDrawCurvedRectangle(x, y, item.width, item.height, tocolor(unpack(backgroundColor)))
				end
			...
			if container.items["gridlist"] then
				for i, item in pairs(container.items["gridlist"]) do
					...
					if item.hovered and maximumScroll > 0 then
						...
						item.scrollToGo = math_max((item.scrollToGo or 0) - (item.blockHeight + item.padding), 0)
						scrolled = true
						globalScrolled = true
					end
				end
			...
			if container.items["gridlist"] then
				for i, item in pairs(container.items["gridlist"]) do
					...
					if item.hovered and maximumScroll > 0 then
						...
						item.scrollTick = getTickCount()
						item.scrollToGo = math_min((item.scrollToGo or 0) + (item.blockHeight + item.padding), maximumScroll)
						scrolled = true
						globalScrolled = true
					end
				end
