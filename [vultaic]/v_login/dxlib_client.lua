...
local defaultFont = {input = dxlib.getFont("RobotoCondensed-Regular", 14), button = dxlib.getFont("Roboto-Medium", 11), gridlist = dxlib.getFont("RobotoCondensed-Regular", 14), selector = dxlib.getFont("RobotoCondensed-Regular", 13)}
	...
	if container.items["input"] then
		for i, item in pairs(container.items["input"]) do
			...
			if y + item.height > 0 and y <= container.height then
				...
				local backgroundColor = item.backgroundColor or {255, 255, 255, 5}
				local fontScale = item.fontScale or dxlib.fontScale
				local font = item.font or dxlib.getFont("Roboto-Medium", 13)
