...
function spawnChanger.requestSpawn(key, state)
	if guiGetInputMode() ~= "allow_binds" or isCursorShowing() then
		return
	end
...
function spawnChanger.requestSpawnExtra(button, press)
	if guiGetInputMode() ~= "allow_binds" or isCursorShowing() or not press or button ~= "arrow_r" and button ~= "arrow_l" then
		return
	end
