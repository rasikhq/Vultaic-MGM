...
float4 ps(vsout input) : COLOR0
{
	...
	float2 position = input.TexCoord.xy - 0.5;
	float x = 0.2 * position.x * sin(300.0 * position.x - 20.0 * time);
	x = 1. / (600. * abs(position.y - x));
	x += 1./length(665.*length(position - float2(0., position.x)));
    float outFX = saturate(opacity * x * intensity);
